...
lines=cv.HoughLinesP(canny,1, np.pi/180, threshold=100, minLineLength=50, maxLineGap=10) #허프 변환을 이용한 선 검출
...
```
cv.HoughLinesP(입력이미지, 거리 해상도, 각도 해상도(rho), 임계값(theta), 최소 선길이, 최대선간격)
허프변환을 이용한 선 검출 방법
